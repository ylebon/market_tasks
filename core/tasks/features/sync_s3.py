...
class Task(TaskStep):
    ...
    def run(self, output_directory=None, bucket_name=None, prefix=None, force=False,
        ...
        if not output_directory:
            output_directory = services_config.get_value('FEATURES.LOCAL_DIRECTORY')
        ...
        # bucket name
        if not bucket_name:
            bucket_name = services_config.get_value('FEATURES.S3_BUCKET').replace("s3://", "")
        ...
        features = parquet_filter.get("features", "*")
        date = parquet_filter.get("date", "*")

        # matching files
        matching_files = list()
        if not features:
            pattern = f"exchange={exchange}/symbol={symbol}/feature=*/date={date}/*.parquet"
            matching_files = fnmatch.filter(files_list, pattern)

        elif isinstance(features, list):
            for feature in features:
                pattern = f"exchange={exchange}/symbol={symbol}/feature={feature}/date={date}/*.parquet"
                matching_files.extend(fnmatch.filter(files_list, pattern))

        elif isinstance(features, str):
            pattern = f"exchange={exchange}/symbol={symbol}/feature={features}/date={date}/*.parquet"
            matching_files = fnmatch.filter(files_list, pattern)

        self.log.info(f"msg='total matching files in S3' pattern='{pattern}' total='{len(matching_files)}'")
